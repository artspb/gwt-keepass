...
import cowj.java.nio.BufferOverflowException;
import cowj.java.nio.BufferUnderflowException;
import cowj.java.nio.ByteBuffer;
import cowj.java.nio.CharBuffer;
import cowj.org.apache.harmony.niochar.internal.nls.Messages;

/**
 * A converter that can converts a 16-bit Unicode character sequence to a byte
 * sequence in some charset.
 * <p>
 * The input character sequence is wrapped by a {@link cowj.java.nio.CharBuffer
 * CharBuffer} and the output character sequence is a
 * {@link cowj.java.nio.ByteBuffer ByteBuffer}. An encoder instance should be used in
 * the following sequence, which is referred to as a encoding operation:
 * <ol>
 ...
 * set to {@code true}</li>
 * <li>invoking the {@link #flush(ByteBuffer) flush} method to flush the output.
 * </li>
 * </ol>
 * <p>
 ...
 * error has happened. A {@link cowj.java.nio.charset.CoderResult CoderResult} instance will be
 ...
 * error to the invoker by a {@link cowj.java.nio.charset.CoderResult CoderResult} instance, and the
 * alternatives are to ignore it or to replace the erroneous input with the
 * replacement byte array. The replacement byte array is '{@code ?}' by default
 * and can be changed by invoking the {@link #replaceWith(byte[]) replaceWith}
 * method. The invoker of this encoder can choose one way by specifying a
 * {@link cowj.java.nio.charset.CodingErrorAction CodingErrorAction} instance for each error type via
 * the {@link #onMalformedInput(cowj.java.nio.charset.CodingErrorAction) onMalformedInput} method and
 * the {@link #onUnmappableCharacter(cowj.java.nio.charset.CodingErrorAction) onUnmappableCharacter}
 * method.
 * <p>
 ...
 * {@link #implFlush(ByteBuffer) implFlush} method and the {@link #implReset()
...
public abstract class CharsetEncoder
{
	 ...
	 * <code>Charset</code>, average number and maximum number of bytes created
	 * by this encoder for one input character.
	 * 
	 * @param cs
	...
	{
		return implCanEncode(CharBuffer.wrap(new char[]
		{
			c
		}));
	}
	 ...
	 * @throws cowj.java.nio.charset.MalformedInputException
	 *             if an illegal input character sequence for this charset is
	 *             encountered, and the action for malformed error is
	 *             {@link cowj.java.nio.charset.CodingErrorAction#REPORT CodingErrorAction.REPORT}
	 * @throws cowj.java.nio.charset.UnmappableCharacterException
	 *             if a legal but unmappable input character sequence for this
	 *             charset is encountered, and the action for unmappable
	 *             character error is {@link cowj.java.nio.charset.CodingErrorAction#REPORT
	 ...
	 * following rules:
	 * <ul>
	 * <li>A {@link cowj.java.nio.charset.CoderResult#malformedForLength(int) malformed input} result
	 ...
	 * number can be got by result's {@link cowj.java.nio.charset.CoderResult#length() length}. This
	 * kind of result can be returned only if the malformed action is
	 * {@link cowj.java.nio.charset.CodingErrorAction#REPORT CodingErrorAction.REPORT}.</li>
	 * <li>{@link cowj.java.nio.charset.CoderResult#UNDERFLOW CoderResult.UNDERFLOW} indicates that as
	 ...
	 * <li>{@link cowj.java.nio.charset.CoderResult#OVERFLOW CoderResult.OVERFLOW} indicates that the
	 ...
	 * <li>A {@link cowj.java.nio.charset.CoderResult#unmappableForLength(int) unmappable character}
	 ...
	 * their number can be got by result's {@link cowj.java.nio.charset.CoderResult#length() length}.
	 ...
	 * {@link #implOnMalformedInput(cowj.java.nio.charset.CodingErrorAction) implOnMalformedInput}
	 ...
	 * 
	 * This method will call the
	 * {@link #implOnUnmappableCharacter(cowj.java.nio.charset.CodingErrorAction)
